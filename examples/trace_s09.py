...
# dependencies = ["httpx>=0.27", "openai>=1.40", "python-dotenv>=1.0"]
# ///
...
Usage (standalone, from the repo root — uv resolves the inline deps, no project venv needed)::
 ...
 # OPENAI_API_KEY in .env at repo root, or: export OPENAI_API_KEY=sk-...
...
def main(argv: list[str]) -> int:
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        print(
            "ERROR: set OPENAI_API_KEY in .env or the environment.",
            file=sys.stderr,
        )
        return 1
